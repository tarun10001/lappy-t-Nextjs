    ...
    return (
      ...
      <div className='bg-white p-6 rounded-lg shadow-md text-center md:text-left'>
        ...
        <div className='text-gray-500 mb-4 flex align-middle justify-center md:justify-start'>
          ...
          <p className='text-orange-700'>
            {laptop.location.street}, {laptop.location.city}{' '}
            {laptop.location.state}
          </p>
        </div>
        ...
        <div className='flex flex-col md:flex-row justify-around'>
          <div className='flex items-center justify-center mb-4 border-b border-gray-200 md:border-b-0 pb-4 md:pb-0'>
            ...
            <div className='text-2xl font-bold'>
              {laptop.rates.nightly ? (
                `$${laptop.rates.nightly.toLocaleString()}`
              ) : (
          ...
          <div className='flex items-center justify-center mb-4 border-b border-gray-200 md:border-b-0 pb-4 md:pb-0'>
            ...
            <div className='text-2xl font-bold text-blue-500'>
              {laptop.rates.weekly ? (
                `$${laptop.rates.weekly.toLocaleString()}`
              ) : (
                <FaTimes className='text-red-700' />
              )}
            </div>
      ...
      <div className='bg-white p-6 rounded-lg shadow-md mt-6'>
        ...
        <div className='flex justify-center gap-4 text-blue-500 mb-4 text-xl space-x-9'>
          <p>
            <FaBed className='inline-block mr-2' /> {laptop.beds}{' '}
            <span className='hidden sm:inline'>Beds</span>
          </p>
          <p>
            <FaBath className='inline-block mr-2' /> {laptop.baths}{' '}
            <span className='hidden sm:inline'>Baths</span>
          </p>
          <p>
            <FaRulerCombined className='inline-block mr-2' />
            {laptop.square_feet} <span className='hidden sm:inline'>sqft</span>
          </p>
        </div>
      ...
      <div className='bg-white p-6 rounded-lg shadow-md mt-6'>
        ...
        <ul className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 list-none space-y-2'>
          {laptop.amenities.map((amenity, index) => (
            <li key={index}>
